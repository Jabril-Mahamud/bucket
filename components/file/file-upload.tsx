...
export function EnhancedFileUpload({
  ...
  const showUploadWarnings =
    isNearLimit("files", 70) || isNearLimit("storage", 70);
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      {showUploadWarnings && (
        <div className="space-y-2">
          {isNearLimit("files", 70) && <UsageWarning type="files" />}
              ...
              {isDragActive ? (
                ...
                <>
                  ...
                  <div className="text-sm text-muted-foreground space-y-1">
                    <p>
                      <strong>Documents:</strong> PDF, DOC, DOCX, TXT, EPUB,
                      RTF, HTML, Markdown
                    </p>
                    <p>
                    ...
                    <p className="text-xs text-blue-600">
                      Documents will be automatically converted to searchable
                      text
                    </p>
                  </div>
                </>
      ...
      {usage && !uploading && (
        <div className="text-xs text-muted-foreground text-center space-y-1">
          <p>
            Total files: {usage.current.totalFiles}/
            {usage.limits.maxFiles === -1 ? "∞" : usage.limits.maxFiles} files,{" "}
            {usage.current.storageGB.toFixed(2)}/{usage.limits.storageGB}GB
            storage
          </p>
          {usage.planName === "free" && getUsagePercentage("files") > 60 && (
            <p className="text-blue-600">
              <Link href="/pricing" className="hover:underline">
                Upgrade for more files and storage →
              </Link>
            </p>
          )}
        </div>
